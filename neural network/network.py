...
class Network:
    ...
    def SGD(self, 
        ...
        for e in range(epoch):            
            ...
            for x in range(int(60000 * 6 / mini_batch_size)):
                nabla_w = [np.zeros(w.shape) for w in self.weights]
                nabla_b = [np.zeros(b.shape) for b in self.bias]

                for x in range(mini_batch_size):
                    label, image = next(t_d)
                    delta_w, delta_b = self.backpropagation(image, label, Cost)

                    nabla_w = [nw + dw for nw, dw in zip(nabla_w, delta_w)]
                    nabla_b = [nb + db for nb, db in zip(nabla_b, delta_b)]
                
                self.weights = [w * (1 - (learning_rate * rp / (60000 * 6))) + mp * mw + (-learning_rate / mini_batch_size * nw) for nw, w, mw in zip(nabla_w, self.weights, nabla_w_p)]
...
if "__main__" == __name__:
    ...
    def image_generator():
        ...
        for i, l in zip(images, label):
            yield (l, np.array([[x] for x in i]))
        ...
        for t in range(1, 6):
            ...
            for image, l in zip(images, label):
               yield (l, np.array([[x] for x in twod2list(offset(random.randint(-5, 5), random.randint(-5, 5), list2_2d(28, image)))]))
    ...
    # print(len(images))
    print(time.time() - s)
    print("images initialised")
    net = Network([28 ** 2, 100, 10], ActivationFunction.sigmoid)
    # net.load_wb("wb.json")



    net.SGD(image_generator, 1, 100, 35, 0, .0, list(zip(t_label, tim)), Cost=CostFunction.CrossEntropy)
